...
import 'Audit.dart';
import 'AuditsAdd.dart';
import 'AuditsEdit.dart';
...
class AuditsList extends StatefulWidget {
  ...
  @override
  State<AuditsList> createState() => _AuditsListState();
}

class _AuditsListState extends State<AuditsList> {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<AuditsProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('Audits Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, AuditsAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<Audit> data) {
  ...
  Widget _buildCard(Audit item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
