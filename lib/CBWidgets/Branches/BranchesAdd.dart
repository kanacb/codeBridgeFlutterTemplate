...
class BranchesAdd extends StatefulWidget {
  const BranchesAdd({super.key, required this.resource});
  final List<Schema> resource;

  @override
  State<BranchesAdd> createState() => _BranchesAddState();
}
...
class _BranchesAddState extends State<BranchesAdd> {
  ...
  void saveForm() async {
    ...
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully added Branches");
    } else {
      snackBar.FailedSnackBar(context, "Failed to add Branches");
    }
  }
