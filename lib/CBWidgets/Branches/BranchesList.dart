...
class _BranchesListState extends State<BranchesList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await BranchesProvider().schema();
    setState(() {});
  }
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('Branches Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, BranchesAdd(resource: schemaResponse?.data)));
            },
