...
class ChataiConfigAdd extends StatefulWidget {
  const ChataiConfigAdd({super.key, required this.resource});
  final List<Schema> resource;

  @override
  State<ChataiConfigAdd> createState() => _ChataiConfigAddState();
}

class _ChataiConfigAddState extends State<ChataiConfigAdd> {
  ...
  void saveForm() async {
    ...
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully added Chatai Config");
    } else {
      snackBar.FailedSnackBar(context, "Failed to add Chatai Config");
    }
  }
