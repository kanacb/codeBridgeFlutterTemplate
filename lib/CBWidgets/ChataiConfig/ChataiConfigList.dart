...
import 'ChataiConfigAdd.dart';
import 'ChataiConfigEdit.dart';
import 'ChataiConfigProvider.dart';


class ChataiConfigList extends StatefulWidget {
  const ChataiConfigList({super.key});

  @override
  State<ChataiConfigList> createState() => _ChataiConfigListState();
}

class _ChataiConfigListState extends State<ChataiConfigList> {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<ChataiConfigProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('ChataiConfig Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, ChataiConfigAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<ChataiConfig> data) {
  ...
  Widget _buildCard(ChataiConfig item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ListTile(
            ...
            subtitle: Column(
              ...
              children: [
                Text('Name: ${item.name}'),
Text('Bedrock Model Id: ${item.bedrockModelId}'),
Text('Model Params Json: ${item.modelParamsJson}'),
Text('Human: ${item.human}'),
Text('Task: ${item.task}'),
Text('No Condition: ${item.noCondition}'),
Text('Yes Condition: ${item.yesCondition}'),
Text('Documents: ${item.documents}'),
Text('Example: ${item.example}'),
Text('Preamble: ${item.preamble}'),
              ],
            ),
            trailing: Row(
                  ...
                  onPressed: () {
                    Navigator.of(context).push(Utils().createRoute(
                        ...
                        ChataiConfigEdit(
                          ...
                          data: item,
                          chatAiEnabler : item.chatAiEnabler,
                        )));
                  },
                ),
                IconButton(
