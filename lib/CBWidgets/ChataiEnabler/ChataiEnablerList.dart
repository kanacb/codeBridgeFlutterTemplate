...
import 'ChataiEnablerAdd.dart';
import 'ChataiEnablerEdit.dart';
import 'ChataiEnablerProvider.dart';


class ChataiEnablerList extends StatefulWidget {
  const ChataiEnablerList({super.key});

  @override
  State<ChataiEnablerList> createState() => _ChataiEnablerListState();
}

class _ChataiEnablerListState extends State<ChataiEnablerList> {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<ChataiEnablerProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('ChataiEnabler Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, ChataiEnablerAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<ChataiEnabler> data) {
  ...
  Widget _buildCard(ChataiEnabler item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
