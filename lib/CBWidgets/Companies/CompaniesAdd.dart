...
class CompaniesAdd extends StatefulWidget {
  const CompaniesAdd({super.key, required this.resource});
  final List<Schema> resource;

  @override
  State<CompaniesAdd> createState() => _CompaniesAddState();
}

class _CompaniesAddState extends State<CompaniesAdd> {
  ...
  void saveForm() async {
    CompaniesProvider provider = CompaniesProvider();
    final data = Companies.fromJson(formData);
    Response response = await provider.createOneAndSave(data);

    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully added Companies");
    } else {
      snackBar.FailedSnackBar(context, "Failed to add Companies");
    }
  }
