...
class CompanyAddressesList extends StatefulWidget {
  const CompanyAddressesList({super.key});

  @override
  State<CompanyAddressesList> createState() => _CompanyAddressesListState();
}

class _CompanyAddressesListState extends State<CompanyAddressesList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await CompanyAddressesProvider().schema();
    setState(() {});
  }

  Future<void> _delete(String id, bool confirm) async {
    if (confirm) {
      final provider = Provider.of<CompanyAddressesProvider>(context, listen: false);
      Response response = await provider.deleteOne(id);

      if (response.isSuccess) {
        SnackBars().SuccessSnackBar(context, "Deleted ticket successfully");
        setState(() {
          provider.loadCompanyAddressesFromHive();
        });
      } else {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<CompanyAddressesProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('CompanyAddresses Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, CompanyAddressesAdd(resource: schemaResponse?.data)));
            },
...
Text('Street 1: ${item.street1}'),
Text('Street 2: ${item.street2}'),
