...
import 'DepartmentAdminAdd.dart';
import 'DepartmentAdminEdit.dart';
import 'DepartmentAdminProvider.dart';


class DepartmentAdminList extends StatefulWidget {
  const DepartmentAdminList({super.key});

  @override
  State<DepartmentAdminList> createState() => _DepartmentAdminListState();
}

class _DepartmentAdminListState extends State<DepartmentAdminList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await DepartmentAdminProvider().schema();
    setState(() {});
  }

  Future<void> _delete(String id, bool confirm) async {
    if (confirm) {
      final provider = Provider.of<DepartmentAdminProvider>(context, listen: false);
      Response response = await provider.deleteOne(id);

      if (response.isSuccess) {
        SnackBars().SuccessSnackBar(context, "Deleted ticket successfully");
        setState(() {
          provider.loadDepartmentAdminFromHive();
        });
      } else {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<DepartmentAdminProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('DepartmentAdmin Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, DepartmentAdminAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<DepartmentAdmin> data) {
  ...
  Widget _buildCard(DepartmentAdmin item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ListTile(
            ...
            subtitle: Column(
              ...
              children: [
                Text('DepartmentId: ${item.departmentId?.deptName}'),
