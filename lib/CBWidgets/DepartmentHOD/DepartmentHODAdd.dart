...
class DepartmentHODAdd extends StatefulWidget {
  const DepartmentHODAdd({super.key, required this.resource});
  final List<Schema> resource;

  @override
  State<DepartmentHODAdd> createState() => _DepartmentHODAddState();
}

class _DepartmentHODAddState extends State<DepartmentHODAdd> {
  ...
  void saveForm() async {
    ...
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully added Department Hod");
    } else {
      snackBar.FailedSnackBar(context, "Failed to add Department Hod");
    }
  }
