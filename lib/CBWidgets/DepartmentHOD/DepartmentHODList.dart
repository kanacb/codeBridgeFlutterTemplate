...
import 'DepartmentHODAdd.dart';
import 'DepartmentHODEdit.dart';
import 'DepartmentHODProvider.dart';


class DepartmentHODList extends StatefulWidget {
  const DepartmentHODList({super.key});

  @override
  State<DepartmentHODList> createState() => _DepartmentHODListState();
}

class _DepartmentHODListState extends State<DepartmentHODList> {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<DepartmentHODProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('DepartmentHOD Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, DepartmentHODAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<DepartmentHOD> data) {
  ...
  Widget _buildCard(DepartmentHOD item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
