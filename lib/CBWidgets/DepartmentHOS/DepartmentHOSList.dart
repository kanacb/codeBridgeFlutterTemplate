...
import 'DepartmentHOSAdd.dart';
import 'DepartmentHOSEdit.dart';
import 'DepartmentHOSProvider.dart';


class DepartmentHOSList extends StatefulWidget {
  const DepartmentHOSList({super.key});

  @override
  State<DepartmentHOSList> createState() => _DepartmentHOSListState();
}

class _DepartmentHOSListState extends State<DepartmentHOSList> {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<DepartmentHOSProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('DepartmentHOS Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, DepartmentHOSAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<DepartmentHOS> data) {
  ...
  Widget _buildCard(DepartmentHOS item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
