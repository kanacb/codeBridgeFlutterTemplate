...
class DepartmentsAdd extends StatefulWidget {
  const DepartmentsAdd({super.key, required this.resource});
  final List<Schema> resource;

  @override
  State<DepartmentsAdd> createState() => _DepartmentsAddState();
}

class _DepartmentsAddState extends State<DepartmentsAdd> {
  ...
  void saveForm() async {
    ...
    final data = Department.fromJson(formData);
    Response response = await provider.createOneAndSave(data);

    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully added Departments");
    } else {
      snackBar.FailedSnackBar(context, "Failed to add Departments");
    }
  }
