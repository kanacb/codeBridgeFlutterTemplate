...
import 'DocumentStorageAdd.dart';
import 'DocumentStorageEdit.dart';
import 'DocumentStorageProvider.dart';


class DocumentStorageList extends StatefulWidget {
  const DocumentStorageList({super.key});

  @override
  State<DocumentStorageList> createState() => _DocumentStorageListState();
}

class _DocumentStorageListState extends State<DocumentStorageList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await DocumentStorageProvider().schema();
    setState(() {});
  }

  Future<void> _delete(String id, bool confirm) async {
    if (confirm) {
      final provider = Provider.of<DocumentStorageProvider>(context, listen: false);
      Response response = await provider.deleteOne(id);

      if (response.isSuccess) {
        SnackBars().SuccessSnackBar(context, "Deleted ticket successfully");
        setState(() {
          provider.loadDocumentStorageFromHive();
        });
      } else {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<DocumentStorageProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('DocumentStorage Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, DocumentStorageAdd(resource: schemaResponse?.data)));
            },
