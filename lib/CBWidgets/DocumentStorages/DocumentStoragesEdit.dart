...
class DocumentStoragesEdit extends StatefulWidget {
  const DocumentStoragesEdit({
    super.key,
  ...
  @override
  State<DocumentStoragesEdit> createState() => _DocumentStoragesEditState();
}

class _DocumentStoragesEditState extends State<DocumentStoragesEdit> {
  ...
  void saveForm() async {
    DocumentStoragesProvider provider = DocumentStoragesProvider();
    // Handle form submission logic here
    final data = DocumentStorage.fromJson(formData);
    Response response = await provider.updateOneAndSave(data.id!, data);
    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully updated Document Storages");
    } else {
      snackBar.FailedSnackBar(context, "Failed to update Document Storages");
    }
  }
