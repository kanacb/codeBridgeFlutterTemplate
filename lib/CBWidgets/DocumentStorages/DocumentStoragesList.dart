...
import 'DocumentStoragesAdd.dart';
import 'DocumentStoragesEdit.dart';
import 'DocumentStoragesProvider.dart';


class DocumentStoragesList extends StatefulWidget {
  const DocumentStoragesList({super.key});

  @override
  State<DocumentStoragesList> createState() => _DocumentStoragesListState();
}

class _DocumentStoragesListState extends State<DocumentStoragesList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await DocumentStoragesProvider().schema();
    setState(() {});
  }

  Future<void> _delete(String id, bool confirm) async {
    if (confirm) {
      final provider = Provider.of<DocumentStoragesProvider>(context, listen: false);
      Response response = await provider.deleteOne(id);

      if (response.isSuccess) {
        SnackBars().SuccessSnackBar(context, "Deleted ticket successfully");
        setState(() {
          provider.loadDocumentStoragesFromHive();
        });
      } else {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<DocumentStoragesProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('DocumentStorages Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, DocumentStoragesAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<DocumentStorage> data) {
  ...
  Widget _buildCard(DocumentStorage item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ListTile(
            ...
            subtitle: Column(
              ...
              children: [
                Text('Size: ${item.size}'),
Text('Last Modified Date: ${item.lastModifiedDate}'),
Text('Last Modified: ${item.lastModified}'),
Text('Table Id: ${item.tableId}'),
Text('Table Name: ${item.tableName}'),
              ],
            ),
            trailing: Row(
              ...
              children: [
                IconButton(
                  ...
                  onPressed: () {
                    Navigator.of(context).push(Utils().createRoute(
                        context,
                        DocumentStoragesEdit(
                          schema: schemaResponse?.data,
                          data: item,
                          name : item.name,path : item.path,eTag : item.eTag,versionId : item.versionId,url : item.url
                        )));
                  },
                ),
                IconButton(
