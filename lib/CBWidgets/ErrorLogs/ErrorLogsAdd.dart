...
class ErrorLogsAdd extends StatefulWidget {
  const ErrorLogsAdd({super.key, required this.resource});
  final List<Schema> resource;

  @override
  State<ErrorLogsAdd> createState() => _ErrorLogsAddState();
}

class _ErrorLogsAddState extends State<ErrorLogsAdd> {
  ...
  void saveForm() async {
    ...
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully added Error Logs");
    } else {
      snackBar.FailedSnackBar(context, "Failed to add Error Logs");
    }
  }
