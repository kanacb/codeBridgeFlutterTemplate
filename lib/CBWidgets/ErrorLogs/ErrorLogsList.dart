...
import 'ErrorLogsAdd.dart';
import 'ErrorLogsEdit.dart';
import 'ErrorLogsProvider.dart';


class ErrorLogsList extends StatefulWidget {
  const ErrorLogsList({super.key});

  @override
  State<ErrorLogsList> createState() => _ErrorLogsListState();
}

class _ErrorLogsListState extends State<ErrorLogsList> {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<ErrorLogsProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('ErrorLogs Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, ErrorLogsAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<ErrorLog> data) {
  ...
  Widget _buildCard(ErrorLog item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
...
Text('Error Message: ${item.errorMessage}'),
Text('Message: ${item.message}'),
Text('Request Body: ${item.requestBody}'),
