...
class FcmQuesAdd extends StatefulWidget {
  const FcmQuesAdd({super.key, required this.resource});
  final List<Schema> resource;

  @override
  State<FcmQuesAdd> createState() => _FcmQuesAddState();
}

class _FcmQuesAddState extends State<FcmQuesAdd> {
  ...
  void saveForm() async {
    FcmQuesProvider provider = FcmQuesProvider();
    final data = FcmQue.fromJson(formData);
    Response response = await provider.createOneAndSave(data);

    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully added Fcm Ques");
    } else {
      snackBar.FailedSnackBar(context, "Failed to add Fcm Ques");
    }
  }
