...
import 'FcmQue.dart';
import 'FcmQuesAdd.dart';
import 'FcmQuesEdit.dart';
import 'FcmQuesProvider.dart';


class FcmQuesList extends StatefulWidget {
  const FcmQuesList({super.key});

  @override
  State<FcmQuesList> createState() => _FcmQuesListState();
}

class _FcmQuesListState extends State<FcmQuesList> {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<FcmQuesProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('FcmQues Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, FcmQuesAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<FcmQue> data) {
  ...
  Widget _buildCard(FcmQue item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
