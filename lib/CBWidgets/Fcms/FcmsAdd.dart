...
class FcmsAdd extends StatefulWidget {
  const FcmsAdd({super.key, required this.resource});
  final List<Schema> resource;

  @override
  State<FcmsAdd> createState() => _FcmsAddState();
}

class _FcmsAddState extends State<FcmsAdd> {
  ...
  void saveForm() async {
    FcmsProvider provider = FcmsProvider();
    final data = Fcm.fromJson(formData);
    Response response = await provider.createOneAndSave(data);

    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully added Fcms");
    } else {
      snackBar.FailedSnackBar(context, "Failed to add Fcms");
    }
  }
