...
import 'HelpBarContentsAdd.dart';
import 'HelpBarContentsEdit.dart';
import 'HelpBarContentsProvider.dart';


class HelpBarContentsList extends StatefulWidget {
  const HelpBarContentsList({super.key});

  @override
  State<HelpBarContentsList> createState() => _HelpBarContentsListState();
}

class _HelpBarContentsListState extends State<HelpBarContentsList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await HelpBarContentsProvider().schema();
    setState(() {});
  }

  Future<void> _delete(String id, bool confirm) async {
    if (confirm) {
      final provider = Provider.of<HelpBarContentsProvider>(context, listen: false);
      Response response = await provider.deleteOne(id);

      if (response.isSuccess) {
        SnackBars().SuccessSnackBar(context, "Deleted ticket successfully");
        setState(() {
          provider.loadHelpBarContentsFromHive();
        });
      } else {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<HelpBarContentsProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('HelpBarContents Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, HelpBarContentsAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<HelpBarContent> data) {
  ...
  Widget _buildCard(HelpBarContent item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
