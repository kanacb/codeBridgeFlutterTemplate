...
import 'HelpSidebarContentsAdd.dart';
import 'HelpSidebarContentsEdit.dart';
import 'HelpSidebarContentsProvider.dart';


class HelpSidebarContentsList extends StatefulWidget {
  const HelpSidebarContentsList({super.key});

  @override
  State<HelpSidebarContentsList> createState() => _HelpSidebarContentsListState();
}

class _HelpSidebarContentsListState extends State<HelpSidebarContentsList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await HelpSidebarContentsProvider().schema();
    setState(() {});
  }

  Future<void> _delete(String id, bool confirm) async {
    if (confirm) {
      final provider = Provider.of<HelpSidebarContentsProvider>(context, listen: false);
      Response response = await provider.deleteOne(id);

      if (response.isSuccess) {
        SnackBars().SuccessSnackBar(context, "Deleted ticket successfully");
        setState(() {
          provider.loadHelpSidebarContentsFromHive();
        });
      } else {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<HelpSidebarContentsProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('HelpSidebarContents Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, HelpSidebarContentsAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<HelpSidebarContent> data) {
  ...
  Widget _buildCard(HelpSidebarContent item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
