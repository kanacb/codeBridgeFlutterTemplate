...
import 'Inbox.dart';
import 'InboxAdd.dart';
import 'InboxEdit.dart';
...
class _InboxListState extends State<InboxList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await InboxProvider().schema();
    setState(() {});
  }
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('Inbox Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, InboxAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<Inbox> data) {
  ...
  Widget _buildCard(Inbox item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
...
Text('To User: ${item.toUser?.name}'),
Text('Subject: ${item.subject}'),
Text('Content: ${item.content}'),
Text('Service: ${item.service}'),
Text('Read: ${item.read}'),
Text('Flagged: ${item.flagged}'),
Text('Sent: ${item.sent}'),
