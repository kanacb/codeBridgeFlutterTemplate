...
class LoginHistoriesEdit extends StatefulWidget {
  const LoginHistoriesEdit({
    super.key,
  ...
  @override
  State<LoginHistoriesEdit> createState() => _LoginHistoriesEditState();
}

class _LoginHistoriesEditState extends State<LoginHistoriesEdit> {
  ...
  void saveForm() async {
    LoginHistoriesProvider provider = LoginHistoriesProvider();
    // Handle form submission logic here
    final data = LoginHistory.fromJson(formData);
    Response response = await provider.updateOneAndSave(data.id!, data);
    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully updated Login Histories");
    } else {
      snackBar.FailedSnackBar(context, "Failed to update Login Histories");
    }
  }
