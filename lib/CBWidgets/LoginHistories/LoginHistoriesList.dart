...
import 'LoginHistoriesAdd.dart';
import 'LoginHistoriesEdit.dart';
import 'LoginHistoriesProvider.dart';


class LoginHistoriesList extends StatefulWidget {
  const LoginHistoriesList({super.key});

  @override
  State<LoginHistoriesList> createState() => _LoginHistoriesListState();
}

class _LoginHistoriesListState extends State<LoginHistoriesList> {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<LoginHistoriesProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('LoginHistories Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, LoginHistoriesAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<LoginHistory> data) {
  ...
  Widget _buildCard(LoginHistory item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ListTile(
            ...
            subtitle: Column(
              ...
              children: [
                Text('UserId: ${item.userId?.name}'),
Text('Device: ${item.device}'),
Text('Browser: ${item.browser}'),
Text('User Agent: ${item.userAgent}'),
Text('loginTime: ${item.loginTime}'),
              ],
            ),
            trailing: Row(
