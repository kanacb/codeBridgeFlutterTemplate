...
class MailQuesAdd extends StatefulWidget {
  const MailQuesAdd({super.key, required this.resource});
  final List<Schema> resource;

  @override
  State<MailQuesAdd> createState() => _MailQuesAddState();
}

class _MailQuesAddState extends State<MailQuesAdd> {
  ...
  void saveForm() async {
    MailQuesProvider provider = MailQuesProvider();
    final data = MailQue.fromJson(formData);
    Response response = await provider.createOneAndSave(data);

    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully added Mail Ques");
    } else {
      snackBar.FailedSnackBar(context, "Failed to add Mail Ques");
    }
  }
