...
import 'MailQue.dart';
import 'MailQuesAdd.dart';
import 'MailQuesEdit.dart';
import 'MailQuesProvider.dart';


class MailQuesList extends StatefulWidget {
  const MailQuesList({super.key});

  @override
  State<MailQuesList> createState() => _MailQuesListState();
}

class _MailQuesListState extends State<MailQuesList> {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<MailQuesProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('MailQues Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, MailQuesAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<MailQue> data) {
  ...
  Widget _buildCard(MailQue item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
...
Text('Recipients: ${item.recipients}'),
Text('Content: ${item.content}'),
Text('payload: ${item.payload}'),
Text('template Id: ${item.templateId}'),
