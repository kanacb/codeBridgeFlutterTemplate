...
class MenuItemsAdd extends StatefulWidget {
  const MenuItemsAdd({super.key, required this.resource});
  final List<Schema> resource;

  @override
  State<MenuItemsAdd> createState() => _MenuItemsAddState();
}

class _MenuItemsAddState extends State<MenuItemsAdd> {
  ...
  void saveForm() async {
    MenuItemsProvider provider = MenuItemsProvider();
    final data = MenuItem.fromJson(formData);
    Response response = await provider.createOneAndSave(data);

    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully added Menu Items");
    } else {
      snackBar.FailedSnackBar(context, "Failed to add Menu Items");
    }
  }
