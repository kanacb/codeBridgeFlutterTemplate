...
import 'MenuItemsAdd.dart';
import 'MenuItemsEdit.dart';
...
class _MenuItemsListState extends State<MenuItemsList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await MenuItemsProvider().schema();
    setState(() {});
  }
  ...
  Future<void> _delete(String id, bool confirm) async {
    if (confirm) {
      ...
      if (response.isSuccess) {
        ...
        setState(() {
          provider.loadMenuItemsFromHive();
        });
      } else {
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('MenuItems Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, MenuItemsAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<MenuItem> data) {
  ...
  Widget _buildCard(MenuItem item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
