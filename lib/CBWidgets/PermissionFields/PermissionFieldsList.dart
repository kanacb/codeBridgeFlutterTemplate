...
import 'PermissionFieldsAdd.dart';
import 'PermissionFieldsEdit.dart';
import 'PermissionFieldsProvider.dart';


class PermissionFieldsList extends StatefulWidget {
  const PermissionFieldsList({super.key});

  @override
  State<PermissionFieldsList> createState() => _PermissionFieldsListState();
}

class _PermissionFieldsListState extends State<PermissionFieldsList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await PermissionFieldsProvider().schema();
    setState(() {});
  }

  Future<void> _delete(String id, bool confirm) async {
    if (confirm) {
      final provider = Provider.of<PermissionFieldsProvider>(context, listen: false);
      Response response = await provider.deleteOne(id);

      if (response.isSuccess) {
        SnackBars().SuccessSnackBar(context, "Deleted ticket successfully");
        setState(() {
          provider.loadPermissionFieldsFromHive();
        });
      } else {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<PermissionFieldsProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('PermissionFields Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, PermissionFieldsAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<PermissionField> data) {
  ...
  Widget _buildCard(PermissionField item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ListTile(
            ...
            subtitle: Column(
              ...
              children: [
                Text('Service Permission Id: ${item.servicePermissionId?.service}'),
Text('Field Name: ${item.fieldName}'),
Text('On Create: ${item.onCreate}'),
Text('On Update: ${item.onUpdate}'),
Text('On Detail: ${item.onDetail}'),
Text('On Table: ${item.onTable}'),
              ],
            ),
            trailing: Row(
