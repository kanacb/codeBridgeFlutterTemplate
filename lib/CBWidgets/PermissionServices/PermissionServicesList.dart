...
import 'PermissionServicesAdd.dart';
import 'PermissionServicesEdit.dart';
import 'PermissionServicesProvider.dart';


class PermissionServicesList extends StatefulWidget {
  const PermissionServicesList({super.key});

  @override
  State<PermissionServicesList> createState() => _PermissionServicesListState();
}

class _PermissionServicesListState extends State<PermissionServicesList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await PermissionServicesProvider().schema();
    setState(() {});
  }

  Future<void> _delete(String id, bool confirm) async {
    if (confirm) {
      final provider = Provider.of<PermissionServicesProvider>(context, listen: false);
      Response response = await provider.deleteOne(id);

      if (response.isSuccess) {
        SnackBars().SuccessSnackBar(context, "Deleted ticket successfully");
        setState(() {
          provider.loadPermissionServicesFromHive();
        });
      } else {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<PermissionServicesProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('PermissionServices Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, PermissionServicesAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<PermissionService> data) {
  ...
  Widget _buildCard(PermissionService item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ListTile(
            ...
            subtitle: Column(
              ...
              children: [
                Text('create: ${item.create}'),
Text('read: ${item.read}'),
Text('update: ${item.update}'),
Text('delete: ${item.delete}'),
Text('import: ${item.import}'),
Text('export: ${item.export}'),
Text('seeder: ${item.seeder}'),
Text('User Id: ${item.userId?.name}'),
Text('Role Id: ${item.roleId?.name}'),
Text('Profile: ${item.profile?.name}'),
Text('Position Id: ${item.positionId?.name}'),
              ],
            ),
            trailing: Row(
