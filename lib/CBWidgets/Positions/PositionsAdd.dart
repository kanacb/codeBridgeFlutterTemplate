...
class PositionsAdd extends StatefulWidget {
  const PositionsAdd({super.key, required this.resource});
  final List<Schema> resource;

  @override
  State<PositionsAdd> createState() => _PositionsAddState();
}

class _PositionsAddState extends State<PositionsAdd> {
  ...
  void saveForm() async {
    PositionsProvider provider = PositionsProvider();
    final data = Positions.fromJson(formData);
    Response response = await provider.createOneAndSave(data);

    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully added Positions");
    } else {
      snackBar.FailedSnackBar(context, "Failed to add Positions");
    }
  }
