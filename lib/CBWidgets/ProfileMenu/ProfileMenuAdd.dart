...
class ProfileMenuAdd extends StatefulWidget {
  const ProfileMenuAdd({super.key, required this.resource});
  final List<Schema> resource;

  @override
  State<ProfileMenuAdd> createState() => _ProfileMenuAddState();
}

class _ProfileMenuAddState extends State<ProfileMenuAdd> {
  ...
  void saveForm() async {
    ProfileMenuProvider provider = ProfileMenuProvider();
    final data = ProfileMenu.fromJson(formData);
    Response response = await provider.createOneAndSave(data);

    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully added Profile Menu");
    } else {
      snackBar.FailedSnackBar(context, "Failed to add Profile Menu");
    }
  }
