...
import 'ProfileMenuAdd.dart';
import 'ProfileMenuEdit.dart';
...
class _ProfileMenuListState extends State<ProfileMenuList> {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<ProfileMenuProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('ProfileMenu Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, ProfileMenuAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<ProfileMenu> data) {
  ...
  Widget _buildCard(ProfileMenu item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
