...
import 'Section.dart';
import 'SectionsAdd.dart';
import 'SectionsEdit.dart';
...
class _SectionsListState extends State<SectionsList> {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<SectionsProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('Sections Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, SectionsAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<Section> data) {
  ...
  Widget _buildCard(Section item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ListTile(
            ...
            subtitle: Column(
              ...
              children: [
                Text('Department: ${item.departmentId?.deptName}'),
Text('Name: ${item.name}'),
Text('Code: ${item.code}'),
