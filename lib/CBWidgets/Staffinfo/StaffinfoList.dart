...
import 'StaffinfoAdd.dart';
import 'StaffinfoEdit.dart';
import 'StaffinfoProvider.dart';


class StaffinfoList extends StatefulWidget {
  const StaffinfoList({super.key});

  @override
  State<StaffinfoList> createState() => _StaffinfoListState();
}

class _StaffinfoListState extends State<StaffinfoList> {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<StaffinfoProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('Staffinfo Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, StaffinfoAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<Staffinfo> data) {
  ...
  Widget _buildCard(Staffinfo item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ListTile(
            ...
            subtitle: Column(
              ...
              children: [
                Text('Empno: ${item.empno}'),
Text('Name: ${item.name}'),
Text('Namenric: ${item.namenric}'),
Text('Compcode: ${item.compcode}'),
Text('Compname: ${item.compname}'),
Text('Deptcode: ${item.deptcode}'),
Text('Deptdesc: ${item.deptdesc}'),
Text('Sectcode: ${item.sectcode}'),
Text('Sectdesc: ${item.sectdesc}'),
Text('Designation: ${item.designation}'),
Text('Email: ${item.email}'),
Text('Resign: ${item.resign}'),
Text('Supervisor: ${item.supervisor}'),
Text('Datejoin: ${item.datejoin}'),
Text('Empgroup: ${item.empgroup}'),
Text('Empgradecode: ${item.empgradecode}'),
Text('Terminationdate: ${item.terminationdate}'),
              ],
            ),
            trailing: Row(
