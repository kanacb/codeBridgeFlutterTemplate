...
class SuperiorsAdd extends StatefulWidget {
  const SuperiorsAdd({super.key, required this.resource});
  final List<Schema> resource;

  @override
  State<SuperiorsAdd> createState() => _SuperiorsAddState();
}

class _SuperiorsAddState extends State<SuperiorsAdd> {
  ...
  void saveForm() async {
    SuperiorsProvider provider = SuperiorsProvider();
    final data = Superior.fromJson(formData);
    Response response = await provider.createOneAndSave(data);

    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully added Superiors");
    } else {
      snackBar.FailedSnackBar(context, "Failed to add Superiors");
    }
  }
