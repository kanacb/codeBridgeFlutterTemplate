...
import 'SuperiorsAdd.dart';
import 'SuperiorsEdit.dart';
import 'SuperiorsProvider.dart';


class SuperiorsList extends StatefulWidget {
  const SuperiorsList({super.key});

  @override
  State<SuperiorsList> createState() => _SuperiorsListState();
}

class _SuperiorsListState extends State<SuperiorsList> {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<SuperiorsProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('Superiors Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, SuperiorsAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<Superior> data) {
  ...
  Widget _buildCard(Superior item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
