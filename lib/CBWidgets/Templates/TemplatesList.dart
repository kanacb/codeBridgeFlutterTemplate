...
class _TemplatesListState extends State<TemplatesList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await TemplatesProvider().schema();
    setState(() {});
  }
  ...
  Future<void> _delete(String id, bool confirm) async {
    if (confirm) {
      ...
      if (response.isSuccess) {
        ...
        setState(() {
          provider.loadTemplatesFromHive();
        });
      } else {
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('Templates Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, TemplatesAdd(resource: schemaResponse?.data)));
            },
