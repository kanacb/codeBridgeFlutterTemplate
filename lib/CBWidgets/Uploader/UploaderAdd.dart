...
class UploaderAdd extends StatefulWidget {
  const UploaderAdd({super.key, required this.resource});
  final List<Schema> resource;

  @override
  State<UploaderAdd> createState() => _UploaderAddState();
}

class _UploaderAddState extends State<UploaderAdd> {
  ...
  void saveForm() async {
    UploaderProvider provider = UploaderProvider();
    final data = Uploader.fromJson(formData);
    Response response = await provider.createOneAndSave(data);

    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully added Uploader");
    } else {
      snackBar.FailedSnackBar(context, "Failed to add Uploader");
    }
  }
