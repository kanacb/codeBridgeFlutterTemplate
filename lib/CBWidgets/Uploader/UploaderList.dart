...
import 'UploaderAdd.dart';
import 'UploaderEdit.dart';
import 'UploaderProvider.dart';


class UploaderList extends StatefulWidget {
  const UploaderList({super.key});

  @override
  State<UploaderList> createState() => _UploaderListState();
}

class _UploaderListState extends State<UploaderList> {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<UploaderProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('Uploader Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, UploaderAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<Uploader> data) {
  ...
  Widget _buildCard(Uploader item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
