...
class UserAddressesAdd extends StatefulWidget {
  const UserAddressesAdd({super.key, required this.resource});
  final List<Schema> resource;

  @override
  State<UserAddressesAdd> createState() => _UserAddressesAddState();
}

class _UserAddressesAddState extends State<UserAddressesAdd> {
  ...
  void saveForm() async {
    UserAddressesProvider provider = UserAddressesProvider();
    final data = UserAddresses.fromJson(formData);
    Response response = await provider.createOneAndSave(data);

    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully added User Addresses");
    } else {
      snackBar.FailedSnackBar(context, "Failed to add User Addresses");
    }
  }
