...
import 'UserAddressesAdd.dart';
import 'UserAddressesEdit.dart';
...
class UserAddressesList extends StatefulWidget {
  ...
  @override
  State<UserAddressesList> createState() => _UserAddressesListState();
}
...
class _UserAddressesListState extends State<UserAddressesList> {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<UserAddressesProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('UserAddresses Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, UserAddressesAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<UserAddress> data) {
  ...
  Widget _buildCard(UserAddress item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
