...
import 'UserChangePasswordAdd.dart';
import 'UserChangePasswordEdit.dart';
import 'UserChangePasswordProvider.dart';


class UserChangePasswordList extends StatefulWidget {
  const UserChangePasswordList({super.key});

  @override
  State<UserChangePasswordList> createState() => _UserChangePasswordListState();
}

class _UserChangePasswordListState extends State<UserChangePasswordList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await UserChangePasswordProvider().schema();
    setState(() {});
  }

  Future<void> _delete(String id, bool confirm) async {
    if (confirm) {
      final provider = Provider.of<UserChangePasswordProvider>(context, listen: false);
      Response response = await provider.deleteOne(id);

      if (response.isSuccess) {
        SnackBars().SuccessSnackBar(context, "Deleted ticket successfully");
        setState(() {
          provider.loadUserChangePasswordFromHive();
        });
      } else {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<UserChangePasswordProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('UserChangePassword Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, UserChangePasswordAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<UserChangePassword> data) {
  ...
  Widget _buildCard(UserChangePassword item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ListTile(
            ...
            subtitle: Column(
              ...
              children: [
                Text('Environment: ${item.environment}'),
Text('Status: ${item.status}'),
Text('Send Email Counter: ${item.sendEmailCounter}'),
Text('Last Attempt: ${item.lastAttempt}'),
              ...
              children: [
                IconButton(
                  ...
                  onPressed: () {
                    Navigator.of(context).push(Utils().createRoute(
                        context,
                        UserChangePasswordEdit(
                          schema: schemaResponse?.data,
                          data: item,
                          userEmail : item.userEmail,server : item.server,code : item.code
                        )));
                  },
                ),
                IconButton(
