...
import 'UserGuideAdd.dart';
import 'UserGuideEdit.dart';
import 'UserGuideProvider.dart';


class UserGuideList extends StatefulWidget {
  const UserGuideList({super.key});

  @override
  State<UserGuideList> createState() => _UserGuideListState();
}

class _UserGuideListState extends State<UserGuideList> {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<UserGuideProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('UserGuide Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, UserGuideAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<UserGuide> data) {
  ...
  Widget _buildCard(UserGuide item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
