...
class UserGuideStepsEdit extends StatefulWidget {
  const UserGuideStepsEdit({
    super.key,
  ...
  final UserGuideStep data;
  final UserGuide? userGuideID;
	final String Steps;
	final String Description;

  @override
  State<UserGuideStepsEdit> createState() => _UserGuideStepsEditState();
}

class _UserGuideStepsEditState extends State<UserGuideStepsEdit> {
  ...
  void saveForm() async {
    UserGuideStepsProvider provider = UserGuideStepsProvider();
    // Handle form submission logic here
    final data = UserGuideStep.fromJson(formData);
    Response response = await provider.updateOneAndSave(data.id!, data);
    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully updated User Guide Steps");
    } else {
      snackBar.FailedSnackBar(context, "Failed to update User Guide Steps");
    }
  }
