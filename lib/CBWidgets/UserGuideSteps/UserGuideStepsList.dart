...
import 'UserGuideStepsAdd.dart';
import 'UserGuideStepsEdit.dart';
import 'UserGuideStepsProvider.dart';


class UserGuideStepsList extends StatefulWidget {
  const UserGuideStepsList({super.key});

  @override
  State<UserGuideStepsList> createState() => _UserGuideStepsListState();
}

class _UserGuideStepsListState extends State<UserGuideStepsList> {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<UserGuideStepsProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('UserGuideSteps Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, UserGuideStepsAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<UserGuideStep> data) {
  ...
  Widget _buildCard(UserGuideStep item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
                  ...
                  onPressed: () {
                    Navigator.of(context).push(Utils().createRoute(
                        ...
                        UserGuideStepsEdit(
                          ...
                          data: item,
                          Steps : item.Steps,Description : item.Description
                        )));
                  },
                ),
                IconButton(
