...
class UserInvitesAdd extends StatefulWidget {
  const UserInvitesAdd({super.key, required this.resource});
  final List<Schema> resource;

  @override
  State<UserInvitesAdd> createState() => _UserInvitesAddState();
}

class _UserInvitesAddState extends State<UserInvitesAdd> {
  ...
  void saveForm() async {
    ...
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully added User Invites");
    } else {
      snackBar.FailedSnackBar(context, "Failed to add User Invites");
    }
  }
