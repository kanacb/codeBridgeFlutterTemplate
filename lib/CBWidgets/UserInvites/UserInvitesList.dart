...
import 'UserInvitesAdd.dart';
import 'UserInvitesEdit.dart';
import 'UserInvitesProvider.dart';


class UserInvitesList extends StatefulWidget {
  const UserInvitesList({super.key});

  @override
  State<UserInvitesList> createState() => _UserInvitesListState();
}

class _UserInvitesListState extends State<UserInvitesList> {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<UserInvitesProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('UserInvites Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, UserInvitesAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<UserInvite> data) {
  ...
  Widget _buildCard(UserInvite item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
...
Text('Role: ${item.role?.name}'),
Text('Company: ${item.company?.name}'),
Text('Branch: ${item.branch?.companyId}'),
Text('Department: ${item.department?.deptName}'),
Text('Section: ${item.section?.name}'),
Text('Code: ${item.code}'),
Text('Send Mail Counter: ${item.sendMailCounter}'),
              ],
            ),
            trailing: Row(
                  ...
                  onPressed: () {
                    Navigator.of(context).push(Utils().createRoute(
                        ...
                        UserInvitesEdit(
                          ...
                          data: item,
                          emailToInvite : item.emailToInvite,
                        )));
                  },
                ),
                IconButton(
