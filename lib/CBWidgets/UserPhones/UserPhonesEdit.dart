...
import 'UserPhonesMain.dart';
  
class UserPhonesEdit extends StatefulWidget {
  const UserPhonesEdit({
    super.key,
  ...
  final UserPhones data;
  final String userId;,

  @override
  State<UserPhonesEdit> createState() => _UserPhonesEditState();
}

class _UserPhonesEditState extends State<UserPhonesEdit> {
  ...
  void saveForm() async {
    UserPhonesProvider provider = UserPhonesProvider();
    // Handle form submission logic here
    final data = UserPhones.fromJson(formData);
    Response response = await provider.updateOneAndSave(data.machineId!, data);
    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully updated User Phones");
    } else {
      snackBar.FailedSnackBar(context, "Failed to update User Phones");
    }
  }
