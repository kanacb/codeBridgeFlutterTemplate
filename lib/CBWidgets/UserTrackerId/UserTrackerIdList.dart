...
import 'UserTrackerIdAdd.dart';
import 'UserTrackerIdEdit.dart';
import 'UserTrackerIdProvider.dart';


class UserTrackerIdList extends StatefulWidget {
  const UserTrackerIdList({super.key});

  @override
  State<UserTrackerIdList> createState() => _UserTrackerIdListState();
}

class _UserTrackerIdListState extends State<UserTrackerIdList> {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<UserTrackerIdProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('UserTrackerId Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, UserTrackerIdAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<UserTrackerId> data) {
  ...
  Widget _buildCard(UserTrackerId item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ListTile(
            ...
            subtitle: Column(
              ...
              children: [
                Text('TrackerCode: ${item.trackerCode}'),
Text('User Agent: ${item.userAgent}'),
Text('Language: ${item.language}'),
Text('Time Zone: ${item.timeZone}'),
Text('Cookeis Enabled: ${item.cookeisEnabled}'),
Text('Do Not Track: ${item.doNotTrack}'),
Text('Hard Concurrency: ${item.hardConcurrency}'),
Text('Market Code: ${item.marketCode}'),
Text('Is logged In: ${item.isLoggedIn}'),
