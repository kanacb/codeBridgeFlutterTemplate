...
class AtlasTicketAdd extends StatefulWidget {
  const AtlasTicketAdd({super.key, required this.resource});
  final List<Schema> resource;

  @override
  State<AtlasTicketAdd> createState() => _AtlasTicketAddState();
}

class _AtlasTicketAddState extends State<AtlasTicketAdd> {
  ...
  void saveForm() async {
    ...
    AtlasTicketProvider atlasTicketProvider = AtlasTicketProvider();
    // Handle form submission logic here
    final data = AtlasTicket.fromJson(formData);
    Response response = await atlasTicketProvider.createOneAndSave(data);
    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully created atlas ticket");
    } else {
      snackBar.FailedSnackBar(context, "Failed to create atlas ticket");
    }
  }
