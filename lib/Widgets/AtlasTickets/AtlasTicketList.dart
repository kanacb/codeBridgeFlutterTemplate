...
import 'package:aims/Widgets/AtlasMachines/AtlasMachines.dart';
import 'package:aims/Widgets/AtlasMachines/AtlasMachinesProvider.dart';
import 'package:aims/Widgets/AtlasTickets/AtlasTicketAbortDialog.dart';
import 'package:aims/Widgets/AtlasTickets/AtlasTicketTechCloseDialog.dart';
import 'package:aims/Widgets/AtlasTickets/AtlasTicketTechOpenDialog.dart';
import 'package:aims/Widgets/Companies/Companies.dart';
import 'package:aims/Widgets/RaiseTicket/ReopenTicket.dart';
...
import 'AtlasTicketAssignTechDialog.dart';
import 'AtlasTicketProvider.dart';
import 'SingleAtlasTicketPage.dart';

class AtlasTicketList extends StatefulWidget {
  final String? successMessage;

  const AtlasTicketList({super.key, this.successMessage});

  @override
  State<AtlasTicketList> createState() => _AtlasTicketListState();
}

class _AtlasTicketListState extends State<AtlasTicketList> {
  final Logger logger = globals.logger;
  final Utils utils = Utils();
  Profile? profile;
  List<AtlasTicket>? filteredTickets;
  List<AtlasMachines>? atlasMachines;
  ...
  Future<void> _loadTickets() async {
    ...
    final ticketProvider = Provider.of<AtlasTicketProvider>(context, listen: false);
    ...
    print("DEBUG AtlasTicketList - tickets: ${filteredTickets!.length}");
  }

  Future<void> _fetchSchema() async {
    await AtlasTicketProvider().fetchAllAndSave();
    schemaResponse = await AtlasTicketProvider().schema();
    if (!mounted) return;
    setState(() {});
  }
  ...
  Future<void> _deleteTicket(String id, bool confirm) async {
    if (confirm) {
      final provider = Provider.of<AtlasTicketProvider>(context, listen: false);
  ...
  Future<List<AtlasTicket>> filterTickets({
    required List<AtlasTicket> unfilteredTickets,
    required String selectedProfilePosition,
    required String selectedProfileId,
  }) async {
    List<AtlasTicket> filteredTickets = unfilteredTickets;
    await _loadAtlasMachines(unfilteredTickets);

    if (!mounted) return [];
    //log(atlasMachines.toString(), name: "provider is working");
    ...
    case "Admin":
      ...
      case "Technician":
        ...
        break;
      case "Vending Controller":
        filteredTickets = unfilteredTickets.where((ticket) => ticket.vendingController?.sId == selectedProfileId).toList();
        break;
      default:
        // unknown role - return empty
        print("AtlasTicketList: unknown role: $selectedProfilePosition");
        filteredTickets = [];
    }
  ...
  List<AtlasTicket> _applyAssignedOnlyFilter(
      List<AtlasTicket> tickets,
  ...
  Future<List<AtlasTicket>> _filterForSupervisor(List<AtlasTicket> tickets, String selectedProfileId) async {
  ...
  Future<bool> _canSupervisorSeeOpenTicket(AtlasTicket ticket) async {
    try {
      // get vending controller profile of each ticket
      Profile? vendingControllerProfile;

      if (ticket.vendingController == null) return false;

      final Response response = await ProfileProvider().fetchOneAndSave(ticket.vendingController!.sId!);
      if (response.error == null) vendingControllerProfile = response.data;
      if (vendingControllerProfile == null) return false;
      ...
      return profile?.branch?.sId == vendingControllerProfile.branch?.sId && selectedProfileCompany?.companyType == "atlas";
    } catch (e) {
  ...
  Future<bool> _isAssignedSupervisorSameBranch(AtlasTicket ticket) async {
    try {
  ...
  Future<void> _loadAtlasMachines(List<AtlasTicket> tickets) async {
    ...
    List<Future<void>> futures = tickets.map((ticket) async {
      try {
        final Response response = await atlasMachinesProvider.fetchOneAndSave(ticket.machineId!);
        if(response.error != null) print("failed fetching atlas machine: ${response.error}");
      } catch (e) {
        print("ERROR _loadAtlasMachines $e");
      }
    }).toList();

    await Future.wait(futures);

    atlasMachines = atlasMachinesProvider.data;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: (profile?.position?.name == "Admin" ||
          ...
          profile?.position?.name == "Vending Controller" ||
        ...
        onPressed: () {
          Navigator.of(context).push(
            MaterialPageRoute(
              builder: (context) => const QrCodeScanner(companyType: "atlas",),
            ),
          );
        },
      )
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('Atlas Ticket Management'),
  ...
  Widget _buildTicketList(List<AtlasTicket> tickets) {
  ...
  Widget _buildTicketCard(AtlasTicket ticket, int index) {
    final atlasMachine = atlasMachines
      ...
      child: InkWell(
        ...
        onTap: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => SingleAtlasTicketPage(ticket: ticket, machine: atlasMachine),
            ),
          );
        },
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  Expanded(
                    child: Text("Serial No: ${atlasMachine?.serialNumber ?? "-"}",
              ...
              Row(
                children: [
                  ...
                  Expanded(child: Text("Model: ${atlasMachine?.modelNo ?? "-"}")),
                ],
              ),
              Row(
                children: [
                  ...
                  const SizedBox(width: 6),
                  Expanded(child: Text("Ownership: ${atlasMachine?.ownership.name ?? "-"}")),
                ],
              ),
              Row(
                children: [
                  ...
                  const SizedBox(width: 6),
                  Expanded(child: Text("Type: ${atlasMachine?.vendingMachineType?.name ?? "-"}")),
                ],
              ),
              Row(
                children: [
                  ...
                  const SizedBox(width: 6),
                  Expanded(child: Text("Vending Controller: ${ticket.vendingController?.name ?? "-"}")),
                ],
              ),
              Row(
                children: [
              ...
              Align(
                ...
                child: (!hasAccess)
                    ? _buildActions(ticket, atlasMachine)
  ...
  Widget _buildShowMoreContent(AtlasTicket ticket, int index) {
  ...
  Widget _buildActions(AtlasTicket ticket, AtlasMachines? machine) {
    List<Widget> actions = [];
    final selectedProfilePosition = profile?.position?.name;
      ...
      actions.add(
        ElevatedButton(
            ...
            onPressed: () {
              showDialog(
                context: context,
                builder: (_) => AtlasTicketAssignTechDialog(
                  ticket: ticket,
        ...
        ElevatedButton(
            ...
            onPressed: () {
              showDialog(
                ...
                builder: (BuildContext dialogContext) => AtlasTicketTechOpenDialog(
                    ticket: ticket,
                    onUpdated: () async {
                      await _loadTickets();
        ...
        ElevatedButton(
            ...
            onPressed: () {
              showDialog(
                context: context,
                barrierDismissible: false,
                builder: (BuildContext dialogContext) => AtlasTicketTechCloseDialog(
                    ticket: ticket,
                    onUpdated: () async {
                      await _loadTickets();
        ...
        ElevatedButton(
            ...
            onPressed: () {
              showDialog(
                context: context,
                barrierDismissible: false,
                builder: (BuildContext dialogContext) => AtlasTicketAbortDialog(
                  ticket: ticket,
                  onUpdated: () async {
                    await _loadTickets();
  ...
  Widget _editDeleteButtons(AtlasTicket ticket) {
    return Row(
