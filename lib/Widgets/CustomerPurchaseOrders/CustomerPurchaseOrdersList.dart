...
import 'CustomerPurchaseOrdersAdd.dart';
import 'CustomerPurchaseOrdersEdit.dart';
import 'CustomerPurchaseOrdersProvider.dart';

class CustomerPurchaseOrdersList extends StatefulWidget {
  const CustomerPurchaseOrdersList({super.key});

  @override
  State<CustomerPurchaseOrdersList> createState() => _CustomerPurchaseOrdersListState();
}

class _CustomerPurchaseOrdersListState extends State<CustomerPurchaseOrdersList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await CustomerPurchaseOrdersProvider().schema();
    setState(() {});
  }

  Future<void> _deleteTicket(String id, bool confirm) async {
    if (confirm) {
      final provider = Provider.of<CustomerPurchaseOrdersProvider>(context, listen: false);
      Response response = await provider.deleteOne(id);

      if (response.isSuccess) {
        SnackBars().SuccessSnackBar(context, "Deleted ticket successfully");
        setState(() {
          provider.loadCustomerPurchaseOrdersFromHive();
        });
      } else {
  ...
  Widget build(BuildContext context) {
    final ticketProvider = Provider.of<CustomerPurchaseOrdersProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('Customer Purchase Orders Page'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, CustomerPurchaseOrdersAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildTicketList(List<CustomerPurchaseOrders> tickets) {
  ...
  Widget _buildTicketCard(CustomerPurchaseOrders ticket, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ListTile(
            ...
            title: Text(
              'Purchase ID: ${ticket.purchaseOrderId}',
              style: const TextStyle(fontWeight: FontWeight.bold),
            ),
            subtitle: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Quotation: ${ticket.quotation}'),
                Text('Purchased Order Date: ${ticket.purchaseOrderDate}'),
                Text('Delivery Date: ${ticket.deliveryDate}'),
              ],
            ),
            trailing: Row(
                  ...
                  onPressed: () {
                    showDialog<String>(
                      ...
                      builder: (context) => DeleteDialog(
                        ...
                        pos: "Yes",
                        neg: "Cancel",
                        id: ticket.purchaseOrderId,
                        answer: (answer) => _deleteTicket(ticket.purchaseOrderId, answer),
                      ),
                    );
                  },
                ),
