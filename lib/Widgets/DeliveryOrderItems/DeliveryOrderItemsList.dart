...
import 'DeliveryOrderItemsAdd.dart';
import 'DeliveryOrderItemsEdit.dart';
import 'DeliveryOrderItemsProvider.dart';

class DeliveryOrderItemsList extends StatefulWidget {
  const DeliveryOrderItemsList({super.key});

  @override
  State<DeliveryOrderItemsList> createState() => _DeliveryOrderItemsListState();
}

class _DeliveryOrderItemsListState extends State<DeliveryOrderItemsList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await DeliveryOrderItemsProvider().schema();
    setState(() {});
  }

  Future<void> _deleteTicket(String id, bool confirm) async {
    if (confirm) {
      final provider = Provider.of<DeliveryOrderItemsProvider>(context, listen: false);
      Response response = await provider.deleteOne(id);

      if (response.isSuccess) {
        SnackBars().SuccessSnackBar(context, "Deleted ticket successfully");
        setState(() {
          provider.loadDeliveryOrderItemsFromHive();
        });
      } else {
  ...
  Widget build(BuildContext context) {
    final ticketProvider = Provider.of<DeliveryOrderItemsProvider>(context);
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, DeliveryOrderItemsAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildTicketList(List<DeliveryOrderItems> tickets) {
  ...
  Widget _buildTicketCard(DeliveryOrderItems ticket, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ListTile(
            ...
            title: Text(
              'Delivery Orders: ${ticket.deliveryOrders}',
                  ...
                  onPressed: () {
                    showDialog<String>(
                      ...
                      builder: (context) => DeleteDialog(
                        ...
                        pos: "Yes",
                        neg: "Cancel",
                        id: ticket.deliveryOrders,
                        answer: (answer) => _deleteTicket(ticket.deliveryOrders, answer),
                      ),
                    );
                  },
                ),
