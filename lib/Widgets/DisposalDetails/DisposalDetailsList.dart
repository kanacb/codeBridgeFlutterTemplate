...
import 'DisposalDetailsAdd.dart';
import 'DisposalDetailsEdit.dart';
import 'DisposalDetailsProvider.dart';

class DisposalDetailsList extends StatefulWidget {
  const DisposalDetailsList({super.key});

  @override
  State<DisposalDetailsList> createState() => _DisposalDetailsListState();
}

class _DisposalDetailsListState extends State<DisposalDetailsList> {
  ...
  Widget build(BuildContext context) {
    final ticketProvider = Provider.of<DisposalDetailsProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('Disposal Details Page'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, DisposalDetailsAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildTicketList(List<DisposalDetails> tickets) {
  ...
  Widget _buildTicketCard(DisposalDetails ticket, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ListTile(
            ...
            subtitle: Column(
              ...
              children: [
                ...
                Text('Associated Number: ${ticket.associatedNumber}'),
                Text('Affective Date: ${ticket.affectiveDate}'),
              ],
            ),
            trailing: Row(
                  ...
                  onPressed: () {
                    showDialog<String>(
                      ...
                      builder: (context) => DeleteDialog(
                        ...
                        pos: "Yes",
                        neg: "Cancel",
                        id: ticket.partNumber,
                        answer: (answer) => _deleteTicket(ticket.partNumber, answer),
                      ),
                    );
                  },
                ),
