...
import 'ExternalChecklistsAdd.dart';
import 'ExternalChecklistsEdit.dart';
import 'ExternalChecklistsProvider.dart';

class ExternalChecklistsList extends StatefulWidget {
  const ExternalChecklistsList({super.key});

  @override
  State<ExternalChecklistsList> createState() => _ExternalChecklistsListState();
}

class _ExternalChecklistsListState extends State<ExternalChecklistsList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await ExternalChecklistsProvider().schema();
    setState(() {});
  }

  Future<void> _deleteChecklist(String id, bool confirm) async {
    if (confirm) {
      final provider = Provider.of<ExternalChecklistsProvider>(context, listen: false);
      Response response = await provider.deleteOne(id);

      if (response.isSuccess) {
        SnackBars().SuccessSnackBar(context, "Deleted checklist successfully");
        setState(() {
          provider.loadExternalChecklistsFromHive(); // Refresh the data
        });
      } else {
  ...
  Widget build(BuildContext context) {
    final checklistsProvider = Provider.of<ExternalChecklistsProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('External Checklists Management'),
  ...
  Widget _buildChecklistList(List<ExternalChecklists> checklists) {
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(utils.createRoute(
                  context, ExternalChecklistsAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildChecklistCard(ExternalChecklists checklist, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ListTile(
            ...
            subtitle: Column(
              ...
              children: [
                Text('Created By: ${checklist.createdBy?.name}'),
                Text('Updated By: ${checklist.updatedBy?.name}'),
