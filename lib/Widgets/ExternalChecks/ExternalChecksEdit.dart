...
class ExternalChecksEdit extends StatefulWidget {
  const ExternalChecksEdit({
    super.key,
    required this.resource,
    required this.check,
  });
  final List<Schema> resource;
  final ExternalChecks check;

  @override
  State<ExternalChecksEdit> createState() => _ExternalChecksEditState();
}

class _ExternalChecksEditState extends State<ExternalChecksEdit> {
  ...
  void saveForm() async {
    ...
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully updated external check");
    } else {
      snackBar.FailedSnackBar(context, "Failed to update external check");
    }
  }
