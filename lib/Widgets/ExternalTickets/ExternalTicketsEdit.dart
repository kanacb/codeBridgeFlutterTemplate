...
class ExternalTicketsEdit extends StatefulWidget {
  const ExternalTicketsEdit({
    super.key,
    required this.resource,
    required this.ticket, required externalTickets,
  });
  final List<Schema> resource;
  final ExternalTickets ticket;

  @override
  State<ExternalTicketsEdit> createState() => _ExternalTicketsEditState();
}

class _ExternalTicketsEditState extends State<ExternalTicketsEdit> {
  ...
  void saveForm() async {
    ...
    ExternalTicketsProvider ticketProvider = ExternalTicketsProvider();
    // Handle form submission logic here
    final data = ExternalTickets.fromJson(formData);
    Response response = await ticketProvider.updateOneAndSave(data.machineId!, data);
    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully updated external ticket");
    } else {
      snackBar.FailedSnackBar(context, "Failed to update external ticket");
    }
  }
