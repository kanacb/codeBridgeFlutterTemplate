...
class RaiseExternalTickets extends StatefulWidget {
  final String qrCode;
  ...
  @override
  _RaiseExternalTicketsState createState() => _RaiseExternalTicketsState();
}

class _RaiseExternalTicketsState extends State<RaiseExternalTickets> {
  ...
  late List<ExternalMachines> machines = [];
  ...
  void submitTicket() async {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      final ExternalTickets ticketData = ExternalTickets(
        ...
        externalUser: IdName(sId: widget.profile.id, name: widget.profile.name),
        endTime: null,
        // salesmanComment: additionalComments, //todo add this after webapp also added
      ...
      try {
        ...
        Response responseTicket = await ExternalTicketsProvider().createOneAndSave(ticketData);
        if (responseTicket.error == null) {
          ExternalTickets ticket = responseTicket.data;
          ...
          if (responseUpload.error == null) {
            ...
            final Response patchedTicket = await ExternalTicketsProvider().patchOneAndSave(ticket.id!, updatedData);
          ...
          // picture upload error
            ...
            await ExternalTicketsProvider().deleteOne(ticket.id!);
        ...
        if (kDebugMode) print("DEBUG RaiseExternalTickets.dart $e");
        if(context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
