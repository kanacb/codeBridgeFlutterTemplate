...
import 'RaiseExternalTickets.dart';
import 'RaiseExternalTicketsProcess.dart';

class SelectedExternalMachine extends StatefulWidget {
  final Barcode? result;
  final String companyType;

  const SelectedExternalMachine({
    super.key,
    this.result,
  ...
  @override
  State<SelectedExternalMachine> createState() => _SelectedExternalMachineState();
}

class _SelectedExternalMachineState extends State<SelectedExternalMachine> {
  final _formKey = GlobalKey<FormState>();
  RaiseExternalTicketsProcess raiseExternalTicketsProcess = RaiseExternalTicketsProcess();
  String serialNumber = "";
  String qrcode = "";
  late List<ExternalMachines>? _machines = [];
  ...
  Future<void> _QrCodeFound(String code) async {
    try {
      Response response = await ExternalMachinesProvider().fetchBySerialNoAndSave(code);
      if (!mounted) return;

      final List<ExternalMachines> machinesFromQR = response.data;
  ...
  void loadChecks(ExternalMachines machine) async {
    dynamic check = await raiseExternalTicketsProcess.ChecklistsNChecks(context, machine);
    checks = check["checks"];
  ...
  Future<void> _handleSearch() async {
    ...
    try {
      Response response = await ExternalMachinesProvider().fetchBySerialNoAndSave(serialNumber);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Select External Machine'),
  ...
  Widget machineTile(ExternalMachines thisMachine) {
    ...
    return InkWell(
      onTap: () {
        Navigator.of(context).pushReplacement(
          MaterialPageRoute(
            builder: (context) => RaiseExternalTickets(
              qrCode: qrcode,
              profile: profile!,
              machine: thisMachine,
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                if (thisMachine.commissionDate != null)
                  Text(
                    "${thisMachine.commissionDate!.month}/${thisMachine.commissionDate!.year}",
                    style: TextStyle(
                      fontSize: 12,
                      color: Colors.grey.shade600,
                    ),
