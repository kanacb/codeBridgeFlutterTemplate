...
class IncomingMachineChecklistsAdd extends StatefulWidget {
  const IncomingMachineChecklistsAdd({super.key, required this.resource});
  final List<Schema> resource;

  @override
  State<IncomingMachineChecklistsAdd> createState() => _IncomingMachineChecklistsAddState();
}

class _IncomingMachineChecklistsAddState extends State<IncomingMachineChecklistsAdd> {
  ...
  void saveForm() async {
    ...
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully added Incoming Machine checklist");
    } else {
      snackBar.FailedSnackBar(context, "Failed to add Incoming Machine checklist");
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Add Incoming Machine Checklist")),
