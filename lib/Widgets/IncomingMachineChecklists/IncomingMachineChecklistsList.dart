...
import 'IncomingMachineChecklistsAdd.dart';
import 'IncomingMachineChecklistsEdit.dart';
import 'IncomingMachineChecklistsProvider.dart';

class IncomingMachineChecklistsList extends StatefulWidget {
  const IncomingMachineChecklistsList({super.key});

  @override
  State<IncomingMachineChecklistsList> createState() => _IncomingMachineChecklistsListState();
}

class _IncomingMachineChecklistsListState extends State<IncomingMachineChecklistsList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await IncomingMachineChecklistsProvider().schema();
    setState(() {});
  }

  Future<void> _deleteCheck(String id, bool confirm) async {
    if (confirm) {
      final provider = Provider.of<IncomingMachineChecklistsProvider>(context, listen: false);
      Response response = await provider.deleteOne(id);

      if (response.isSuccess) {
        SnackBars().SuccessSnackBar(context, "Deleted check successfully");
        setState(() {
          provider.loadIncomingMachineChecklistsFromHive(); // Refresh the data
        });
      } else {
  ...
  Widget build(BuildContext context) {
    final checksProvider = Provider.of<IncomingMachineChecklistsProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('Incoming Machine Checks Management'),
  ...
  Widget _buildCheckList(List<IncomingMachineChecklists> checks) {
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(utils.createRoute(
                  context, IncomingMachineChecklistsAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildCheckCard(IncomingMachineChecklists check, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ...
          _showMore[index]
            ...
            child: Text('Checklist ID: ${check.id ?? "N/A"}'),
          )
              : const SizedBox(),
          TextButton(
