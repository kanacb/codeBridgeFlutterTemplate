...
import 'IncomingMachineChecksAdd.dart';
import 'IncomingMachineChecksEdit.dart';
import 'IncomingMachineChecksProvider.dart';

class IncomingMachineChecksList extends StatefulWidget {
  const IncomingMachineChecksList({super.key});

  @override
  State<IncomingMachineChecksList> createState() => _IncomingMachineChecksListState();
}

class _IncomingMachineChecksListState extends State<IncomingMachineChecksList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await IncomingMachineChecksProvider().schema();
    setState(() {});
  }

  Future<void> _deleteCheck(String id, bool confirm) async {
    if (confirm) {
      final provider = Provider.of<IncomingMachineChecksProvider>(context, listen: false);
      Response response = await provider.deleteOne(id);

      if (response.isSuccess) {
        SnackBars().SuccessSnackBar(context, "Deleted check successfully");
        setState(() {
          provider.loadIncomingMachineChecksFromHive(); // Refresh the data
        });
      } else {
  ...
  Widget build(BuildContext context) {
    final checksProvider = Provider.of<IncomingMachineChecksProvider>(context);
  ...
  Widget _buildCheckList(List<IncomingMachineChecks> checks) {
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(utils.createRoute(
                  context, IncomingMachineChecksAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildCheckCard(IncomingMachineChecks check, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ListTile(
            ...
            subtitle: Column(
              ...
              children: [
                Text('Checklist ID: ${check.checkListId}'),
