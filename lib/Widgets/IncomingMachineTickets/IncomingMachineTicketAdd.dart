...
class IncomingMachineTicketAdd extends StatefulWidget {
  const IncomingMachineTicketAdd({super.key, required this.resource});
  final List<Schema> resource;

  @override
  State<IncomingMachineTicketAdd> createState() => _IncomingMachineTicketAddState();
}

class _IncomingMachineTicketAddState extends State<IncomingMachineTicketAdd> {
  ...
  void saveForm() async {
    ...
    IncomingMachineTicketProvider incomingMachineTicketProvider = IncomingMachineTicketProvider();
    // Handle form submission logic here
    final data = IncomingMachineTicket.fromJson(formData);
    Response response = await incomingMachineTicketProvider.createOneAndSave(data);
    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully created IncomingMachine ticket");
    } else {
      snackBar.FailedSnackBar(context, "Failed to create IncomingMachine ticket");
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          "Add new IncomingMachine ticket",
