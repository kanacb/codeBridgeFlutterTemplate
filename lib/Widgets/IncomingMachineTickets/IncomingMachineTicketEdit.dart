...
class IncomingMachineTicketEdit extends StatefulWidget {
  const IncomingMachineTicketEdit({
    super.key,
    required this.resource,
    required this.ticket, required IncomingMachineTicket, required incomingMachineTicket,
  });
  final List<Schema> resource;
  final IncomingMachineTicket ticket;

  @override
  State<IncomingMachineTicketEdit> createState() => _IncomingMachineTicketEditState();
}

class _IncomingMachineTicketEditState extends State<IncomingMachineTicketEdit> {
  ...
  void saveForm() async {
    ...
    IncomingMachineTicketProvider ticketProvider = IncomingMachineTicketProvider();
    // Handle form submission logic here
    final data = IncomingMachineTicket.fromJson(formData);
    Response response = await ticketProvider.updateOneAndSave(data.machineId!, data);
    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully updated IncomingMachine ticket");
    } else {
      snackBar.FailedSnackBar(context, "Failed to update IncomingMachine ticket");
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          "Edit Ticket - ${widget.ticket.machineId}",
