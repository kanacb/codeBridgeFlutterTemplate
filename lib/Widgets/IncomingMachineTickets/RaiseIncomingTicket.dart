...
class RaiseIncomingTicket extends StatefulWidget {
  final String qrCode;
  ...
  @override
  _RaiseIncomingTicketState createState() => _RaiseIncomingTicketState();
}

class _RaiseIncomingTicketState extends State<RaiseIncomingTicket> {
  ...
  void submitTicket() async {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      String companyType = Methods.getCompanyFromMachine(widget.machine)?.companyType ?? "";
      final Map<String, String> serviceMap = {
        "atlas": "atlas_machines",
        "etika": "mem_machines",
        "external": "external_machines",
        "irms": "irms_machines",
      };
      // write the function to save the ticket.
      final IncomingMachineTicket ticketData = IncomingMachineTicket(
        machineId:widget.machine.id!,
        machineService: serviceMap[companyType.toLowerCase()],
        vendingControllerChecklistResponse: _checklistResponse,
        selectedJobStations: [],
        status: "Open",
        startTime: DateTime.now().toUtc(),
        incomingMachineChecker: IdName(sId: widget.profile.id, name: widget.profile.name),
        endTime: null,
