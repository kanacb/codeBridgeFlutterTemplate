...
class JobStationsEdit extends StatefulWidget {
  const JobStationsEdit({
    super.key,
    required this.resource,
    required this.ticket, required jobStations,
  });
  final List<Schema> resource;
  final JobStations ticket;

  @override
  State<JobStationsEdit> createState() => _JobStationsEditState();
}

class _JobStationsEditState extends State<JobStationsEdit> {
  ...
  void saveForm() async {
    ...
    JobStationsProvider jobStationsProvider = JobStationsProvider();
    // Handle form submission logic here
    final data = JobStations.fromJson(formData);
    Response response = await jobStationsProvider.updateOneAndSave(data.name!, data);
    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully updated Job Station ");
    } else {
      snackBar.FailedSnackBar(context, "Failed to update Job Station ");
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          "Edit Job Station - ${widget.ticket.name}",
