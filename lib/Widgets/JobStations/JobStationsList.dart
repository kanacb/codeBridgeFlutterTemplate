...
import 'JobStationsAdd.dart';
import 'JobStationsEdit.dart';
import 'JobStationsProvider.dart';

class JobStationsList extends StatefulWidget {
  const JobStationsList({super.key});

  @override
  State<JobStationsList> createState() => _JobStationsListState();
}

class _JobStationsListState extends State<JobStationsList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await JobStationsProvider().schema();
    setState(() {});
  }

  Future<void> _deleteTicket(String id, bool confirm) async {
    if (confirm) {
      final provider = Provider.of<JobStationsProvider>(context, listen: false);
      Response response = await provider.deleteOne(id);

      if (response.isSuccess) {
        SnackBars().SuccessSnackBar(context, "Deleted job stations successfully");
        setState(() {
          provider.loadJobStationsFromHive();
        });
      } else {
        SnackBars().FailedSnackBar(context, "Failed to delete job stations");
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    final ticketProvider = Provider.of<JobStationsProvider>(context);
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, JobStationsAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildTicketList(List<JobStations> tickets) {
  ...
  Widget _buildTicketCard(JobStations ticket, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
                  ...
                  onPressed: () {
                    Navigator.of(context).push(Utils().createRoute(
                        ...
                        JobStationsEdit(
                          ...
                          ticket: ticket, jobStations: null,
                        )));
                  },
                ),
                IconButton(
                  ...
                  onPressed: () {
                    showDialog<String>(
                      ...
                      builder: (context) => DeleteDialog(
                        ...
                        neg: "Cancel",
                        id: ticket.name,
                        answer: (answer) => _deleteTicket(ticket.name, answer),
                      ),
                    );
                  },
                ),
