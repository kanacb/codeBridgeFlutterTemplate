...
class MachineMasterEdit extends StatefulWidget {
  const MachineMasterEdit({
    super.key,
  ...
  @override
  State<MachineMasterEdit> createState() => _MachineMasterEditState();
}

class _MachineMasterEditState extends State<MachineMasterEdit> {
  ...
  void initState() {
    super.initState();
    dynamic initialValue = widget.machine.toJson();
  ...
  void saveForm() async {
    ...
    MachineMasterProvider machineProvider = MachineMasterProvider();
    // Handle form submission logic here
    final data = MachineMaster.fromJson(formData);
    Response response =
    await machineProvider.updateOneAndSave(data.serialNumber!, data);
    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully updated machine master");
    } else {
      snackBar.FailedSnackBar(context, "Failed to update machine master");
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          "Edit Machine - ${widget.machine.serialNumber}",
