...
import 'MachineMasterAdd.dart';
import 'MachineMasterEdit.dart';
import 'MachineMasterProvider.dart';

class MachineMasterList extends StatefulWidget {
  const MachineMasterList({super.key});

  @override
  State<MachineMasterList> createState() => _MachineMasterListState();
}

class _MachineMasterListState extends State<MachineMasterList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await MachineMasterProvider().schema();
  ...
  Widget build(BuildContext context) {
    final machineProvider =
        Provider.of<MachineMasterProvider>(context, listen: false);
    List<MachineMaster> machines = machineProvider.data.toList();
  ...
  Widget _buildMachineList(List<MachineMaster> machines) {
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(utils.createRoute(
                  context, MachineMasterAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildMachineCard(MachineMaster machine, int index) {
    return Card(
      ...
      child: Column(
        children: [
          ListTile(
            ...
            title: Text(
              machine.serialNumber!.substring(0, 10).padRight(11," ..."),
                  ...
                  onPressed: () {
                    showDialog<String>(
                      ...
                      builder: (context) => DeleteDialog(
                        ...
                        answer: (answer) =>
                            _deleteMachine(machine.serialNumber!, answer),
                      ),
                    );
                  },
                ),
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(machine.serialNumber!, style: TextStyle(fontSize: 20), maxLines: 3, textAlign: TextAlign.center,),
                Text(
                  'Model:',
                  style: TextStyle(fontWeight: FontWeight.bold),
                ),
                Text(machine.modelNo!),
                Text(
                  'Ownership:',
                  style: TextStyle(fontWeight: FontWeight.bold),
                ),
                Text(machine.ownership.name),
                Text(
                  'VMCode:',
                  style: TextStyle(fontWeight: FontWeight.bold),
                ),
                Text(machine.vendingMachineCode!),
                Text(
                  'Type:',
                  style: TextStyle(fontWeight: FontWeight.bold),
                ),
                Text(machine.vendingMachineType!.name!),
                SizedBox(
                  height: 30,
                ),
              ],
