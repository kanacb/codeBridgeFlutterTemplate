...
class RaiseMemTicket extends StatefulWidget {
  ...
  @override
  _RaiseMemTicketState createState() => _RaiseMemTicketState();
}

class _RaiseMemTicketState extends State<RaiseMemTicket> {
  ...
  late List<MemMachines> machines = [];
  ...
  void submitTicket() async {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      final MemTicket ticketData = MemTicket(
        ...
        salesman: IdName(sId: widget.profile.id, name: widget.profile.name),
        endTime: null,
        salesmanComment: additionalComments,
      ...
      try {
        ...
        Response responseTicket = await MemTicketProvider().createOneAndSave(ticketData);
        if (responseTicket.error == null) {
          MemTicket ticket = responseTicket.data;
          ...
          if (responseUpload.error == null) {
            ...
            final Response patchedTicket = await MemTicketProvider().patchOneAndSave(ticket.id!, updatedData);
          ...
          // picture upload error
            ...
            await MemTicketProvider().deleteOne(ticket.id!);
        ...
        if (kDebugMode) print("DEBUG RaiseMemTicket.dart $e");
        if(context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
