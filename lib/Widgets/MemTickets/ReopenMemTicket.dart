...
import 'MemTicketPage.dart';
import 'MemTicketProvider.dart';
import 'RaiseMemTicketProcess.dart';

class ReopenMemTicket extends StatefulWidget {
  final MemTicket oldTicket;
  final Profile newVC;

  const ReopenMemTicket({
    super.key,
  ...
  @override
  State<ReopenMemTicket> createState() => _ReopenMemTicketState();
}

class _ReopenMemTicketState extends State<ReopenMemTicket> {
  final GlobalKey<FormState> _formKey = GlobalKey<FormState>();
  late final User _user;
  MemMachines? _machine;
  ...
  final RaiseMemTicketProcess raiseMemTicketProcess = RaiseMemTicketProcess();
  ...
  void init() async {
    ...
    Response response = await MemMachinesProvider().fetchOneAndSave(widget.oldTicket.machineId!);
  ...
  void submitTicket() async {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      final MemTicket ticketData = MemTicket(
        machineId:widget.oldTicket.machineId,
        checklistResponse: _selectedIssues,
        assignedTechnician: null,
        status: "Open",
        startTime: DateTime.now().toUtc(),
        salesman: IdName(sId: widget.newVC.id, name: widget.newVC.name),
        endTime: null,
        salesmanComment: _additionalComments,
      ...
      try {
        ...
        Response newTicketResponse = await MemTicketProvider().createOneAndSave(ticketData);
        if (newTicketResponse.error != null) {
          _showError(context, newTicketResponse.error!);
          return;
        }
        MemTicket newTicket = newTicketResponse.data;
        ...
        if (uploadResponse.error != null) {
          await MemTicketProvider().deleteOne(newTicket.id!);
        ...
        final Response newTicketPatchedResponse = await MemTicketProvider().patchOneAndSave(newTicket.id!, newTicketImageUpdate);
        ...
        final Response oldTicketPatchedResponse = await MemTicketProvider().patchOneAndSave(widget.oldTicket.id!, oldTicketReopenUpdate);
  ...
  Future<void> loadChecks(MemMachines thisMachine) async {
    dynamic check = await raiseMemTicketProcess.ChecklistsNChecks(context, thisMachine);
    ...
    return Column(
      ...
      _checklists!.map((checklist) {
        final List checklistChecks = _checks!
            .where((check) => check.memCheckListId == checklist.id)
