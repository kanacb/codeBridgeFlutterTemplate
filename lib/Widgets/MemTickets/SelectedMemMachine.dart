...
import 'RaiseMemTicket.dart';
import 'RaiseMemTicketProcess.dart';

class SelectedMemMachine extends StatefulWidget {
  final Barcode? result;
  final String companyType;

  const SelectedMemMachine({
    super.key,
    this.result,
  ...
  @override
  State<SelectedMemMachine> createState() => _SelectedMemMachineState();
}

class _SelectedMemMachineState extends State<SelectedMemMachine> {
  final _formKey = GlobalKey<FormState>();
  RaiseMemTicketProcess raiseMemTicketProcess = RaiseMemTicketProcess();
  String serialNumber = "";
  String qrcode = "";
  late List<MemMachines>? _machines = [];
  ...
  Future<void> _QrCodeFound(String code) async {
    try {
      Response response = await MemMachinesProvider().fetchBySerialNoAndSave(code);
      if (!mounted) return;

      final List<MemMachines> machinesFromQR = response.data;
  ...
  void loadChecks(MemMachines machine) async {
    dynamic check = await raiseMemTicketProcess.ChecklistsNChecks(context, machine);
    checks = check["checks"];
  ...
  Future<void> _handleSearch() async {
    ...
    try {
      Response response = await MemMachinesProvider().regex('serialNumber', serialNumber);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Select MEM Machine'),
  ...
  Widget machineTile(MemMachines thisMachine) {
    ...
    return InkWell(
      onTap: () {
        Navigator.of(context).pushReplacement(
          MaterialPageRoute(
            builder: (context) => RaiseMemTicket(
              qrCode: qrcode,
              profile: profile!,
              machine: thisMachine,
