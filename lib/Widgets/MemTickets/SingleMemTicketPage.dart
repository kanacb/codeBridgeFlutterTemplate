...
import '../MemParts/MemParts.dart';
import '../MemParts/MemPartsProvider.dart';
import 'MemTicket.dart';

class SingleMemTicketPage extends StatefulWidget {
  final MemTicket ticket;
  final MemMachines? machine;

  const SingleMemTicketPage({
    super.key,
  ...
  @override
  State<SingleMemTicketPage> createState() => _SingleMemTicketPageState();
}

class _SingleMemTicketPageState extends State<SingleMemTicketPage> {

  late final MemTicket _ticket;
  late final MemMachines? _machine;
  ...
  Future<void> _loadDocumentStorages(MemTicket ticket) async {
  ...
  Future<void> _loadUsedParts() async {
    if (_ticket.usedParts == null || _ticket.usedParts!.isEmpty) {
      return;
    }

    List<MemParts> memParts = [];

    for (var usedPart in _ticket.usedParts!) {
      if (usedPart.partId != null) {
        try {
          // Fetch the part details
          Response response = await MemPartsProvider().fetchOneAndSave(usedPart.partId!);
          if (response.error == null && response.data != null) {
            MemParts part = response.data;
            memParts.add(part);
          }
        } catch (e) {
          print('Error fetching part ${usedPart.partId}: $e');
        }
      }
    }

    setState(() {
      _usedMemParts = memParts;
    });
  }
  ...
  void _copyPageLink() {
    final link = 'app://memTickets/${widget.ticket.id}';
                ...
                if (constraints.maxWidth > 900) {
                  return Row(
                    ...
                    children: [
                      ...
                      Expanded(child: _buildSalesmanCard()),
                      const SizedBox(width: 16),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            _buildInfoRow('Salesman', _ticket.salesman?.name ?? 'N/A'),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildInfoRow('Initial Comment', _ticket.openingRemarks?.isNotEmpty == true
                  ? _ticket.openingRemarks!
                  : 'No comment available'),
              _buildInfoRow('Comment', _ticket.closingRemarks?.isNotEmpty == true
                  ? _ticket.closingRemarks!
                  : 'No comment available'),
              _buildUsedPartsTable(),
            ],
          ),
        ),
      ),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          if (_ticket.usedParts != null && _ticket.usedParts!.isNotEmpty)
            Container(
              ...
              child: Column(
                children: [
                  ...
                  // Data rows
                  ..._ticket.usedParts!.asMap().entries.map((entry) {
                    final index = entry.key;
                    final usedPart = entry.value;

                    // Find the matching MemParts object
                    final memPart = _usedMemParts.where(
                          (part) => part.id == usedPart.partId
                    ).firstOrNull;
                      ...
                      child: Row(
                        children: [
                          Expanded(
                            flex: 2,
                            child: Text(
                              memPart?.item ?? "Unknown Part",
                              style: const TextStyle(fontSize: 14),
                            ),
                          ),
                          Expanded(
                            child: Text(
                              usedPart.quantity?.toString() ?? "0",
                              style: const TextStyle(fontSize: 14),
                            ),
                          ),
                        ],
