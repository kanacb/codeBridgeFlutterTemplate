...
import 'PartRequestDetailsAdd.dart';
import 'PartRequestDetailsEdit.dart';
import 'PartRequestDetailsProvider.dart';

class PartRequestDetailsList extends StatefulWidget {
  const PartRequestDetailsList({super.key});

  @override
  State<PartRequestDetailsList> createState() => _PartRequestDetailsListState();
}

class _PartRequestDetailsListState extends State<PartRequestDetailsList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await PartRequestDetailsProvider().schema();
    setState(() {});
  }

  Future<void> _deleteTicket(String id, bool confirm) async {
    if (confirm) {
      final provider = Provider.of<PartRequestDetailsProvider>(context, listen: false);
      Response response = await provider.deleteOne(id);

      if (response.isSuccess) {
        SnackBars().SuccessSnackBar(context, "Deleted ticket successfully");
        setState(() {
          provider.loadPartRequestDetailsFromHive();
        });
      } else {
  ...
  Widget build(BuildContext context) {
    final ticketProvider = Provider.of<PartRequestDetailsProvider>(context);
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, PartRequestDetailsAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildTicketList(List<PartRequestDetails> tickets) {
  ...
  Widget _buildTicketCard(PartRequestDetails ticket, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ListTile(
            ...
            subtitle: Column(
              ...
              children: [
                ...
                Text('Requested Date: ${ticket.requestedDate}'),
                Text('Job ID: ${ticket.jobId}'),
                Text('Technician: ${ticket.technician}'),
              ],
            ),
            trailing: Row(
