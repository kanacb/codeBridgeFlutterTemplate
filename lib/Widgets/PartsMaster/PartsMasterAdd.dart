...
class PartsMasterAdd extends StatefulWidget {
  const PartsMasterAdd({super.key, required this.resource});
  final List<Schema> resource;

  @override
  State<PartsMasterAdd> createState() => _PartsMasterAddState();
}

class _PartsMasterAddState extends State<PartsMasterAdd> {
  ...
  void saveForm() async {
    ...
    PartsMasterProvider partsMasterProvider = PartsMasterProvider();
    // Handle form submission logic here
    final data = PartsMaster.fromJson(formData);
    Response response = await partsMasterProvider.createOneAndSave(data);
    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully created Parts master");
    } else {
      snackBar.FailedSnackBar(context, "Failed to create Parts master");
    }
  }
