...
class PartsMasterEdit extends StatefulWidget {
  const PartsMasterEdit({
    super.key,
    required this.resource,
    required this.parts,
  });
  final List<Schema> resource;
  final PartsMaster parts;

  @override
  State<PartsMasterEdit> createState() => _PartsMasterEditState();
}

class _PartsMasterEditState extends State<PartsMasterEdit> {
  ...
  void initState() {
    super.initState();
    dynamic initialValue = widget.parts.toJson();
  ...
  void saveForm() async {
    ...
    PartsMasterProvider partsProvider = PartsMasterProvider();
    // Handle form submission logic here
    final data = PartsMaster.fromJson(formData);
    Response response =
    await partsProvider.updateOneAndSave(data.itemNo!, data);
    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully updated Parts master");
    } else {
      snackBar.FailedSnackBar(context, "Failed to update Parts master");
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          "Edit Parts - ${widget.parts.itemNo}",
