...
import 'PurchaseOrderItemsAdd.dart';
import 'PurchaseOrderItemsEdit.dart';
import 'PurchaseOrderItemsProvider.dart';

class PurchaseOrderItemsList extends StatefulWidget {
  const PurchaseOrderItemsList({super.key});

  @override
  State<PurchaseOrderItemsList> createState() => _PurchaseOrderItemsListState();
}

class _PurchaseOrderItemsListState extends State<PurchaseOrderItemsList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await PurchaseOrderItemsProvider().schema();
    setState(() {});
  }

  Future<void> _deleteTicket(String id, bool confirm) async {
    if (confirm) {
      final provider = Provider.of<PurchaseOrderItemsProvider>(context, listen: false);
      Response response = await provider.deleteOne(id);

      if (response.isSuccess) {
        SnackBars().SuccessSnackBar(context, "Deleted ticket successfully");
        setState(() {
          provider.loadPurchaseOrderItemsFromHive();
        });
      } else {
  ...
  Widget build(BuildContext context) {
    final ticketProvider = Provider.of<PurchaseOrderItemsProvider>(context);
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, PurchaseOrderItemsAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildTicketList(List<PurchaseOrderItems> tickets) {
  ...
  Widget _buildTicketCard(PurchaseOrderItems ticket, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ListTile(
            ...
            title: Text(
              'Purchase Order: ${ticket.purchaseOrder}',
                  ...
                  onPressed: () {
                    showDialog<String>(
                      ...
                      builder: (context) => DeleteDialog(
                        ...
                        pos: "Yes",
                        neg: "Cancel",
                        id: ticket.purchaseOrder,
                        answer: (answer) => _deleteTicket(ticket.purchaseOrder, answer),
                      ),
                    );
                  },
                ),
