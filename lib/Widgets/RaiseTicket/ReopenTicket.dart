...
import 'RaiseTicketProcess.dart';

class ReopenTicket extends StatefulWidget {
  final AtlasTicket oldTicket;
  final Profile newVC;
...
class _ReopenTicketState extends State<ReopenTicket> {
  ...
  final RaiseTicketProcess raiseTicketProcess = RaiseTicketProcess();
  ...
  void init() async {
    ...
    Response response = await AtlasMachinesProvider().fetchOneAndSave(widget.oldTicket.machineId!);
  ...
  void submitTicket() async {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      final AtlasTicket ticketData = AtlasTicket(
        ...
        vendingController: IdName(sId: widget.newVC.id, name: widget.newVC.name),
        endTime: null,
      ...
      try {
        ...
        Response newTicketResponse = await AtlasTicketProvider().createOneAndSave(ticketData);
        if (newTicketResponse.error != null) {
          _showError(context, newTicketResponse.error!);
          return;
        }
        AtlasTicket newTicket = newTicketResponse.data;
        ...
        if (uploadResponse.error != null) {
          await AtlasTicketProvider().deleteOne(newTicket.id!);
        ...
        final Response newTicketPatchedResponse = await AtlasTicketProvider().patchOneAndSave(newTicket.id!, newTicketImageUpdate);
        ...
        final Response oldTicketPatchedResponse = await AtlasTicketProvider().patchOneAndSave(widget.oldTicket.id!, oldTicketReopenUpdate);
  ...
  Future<void> loadChecks(AtlasMachines thisMachine) async {
