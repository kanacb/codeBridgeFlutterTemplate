...
import 'SalesOrderItemsAdd.dart';
import 'SalesOrderItemsEdit.dart';
import 'SalesOrderItemsProvider.dart';

class SalesOrderItemsList extends StatefulWidget {
  const SalesOrderItemsList({super.key});

  @override
  State<SalesOrderItemsList> createState() => _SalesOrderItemsListState();
}

class _SalesOrderItemsListState extends State<SalesOrderItemsList> {
  ...
  Widget build(BuildContext context) {
    final ticketProvider = Provider.of<SalesOrderItemsProvider>(context);
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, SalesOrderItemsAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildTicketList(List<SalesOrderItems> tickets) {
  ...
  Widget _buildTicketCard(SalesOrderItems ticket, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
