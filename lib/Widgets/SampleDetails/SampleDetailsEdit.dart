...
class SampleDetailsEdit extends StatefulWidget {
  const SampleDetailsEdit({
    super.key,
    required this.resource,
    required this.ticket, required jobStationTicket,
  });
  final List<Schema> resource;
  final SampleDetails ticket;

  @override
  State<SampleDetailsEdit> createState() => _SampleDetailsEditState();
}

class _SampleDetailsEditState extends State<SampleDetailsEdit> {
  ...
  void saveForm() async {
    ...
    SampleDetailsProvider ticketProvider = SampleDetailsProvider();
    // Handle form submission logic here
    final data = SampleDetails.fromJson(formData);
    Response response = await ticketProvider.updateOneAndSave(data.partNumber!, data);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          "Edit Ticket - ${widget.ticket.partNumber}",
