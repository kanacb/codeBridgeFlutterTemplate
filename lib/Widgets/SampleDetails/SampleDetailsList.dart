...
import 'SampleDetailsAdd.dart';
import 'SampleDetailsEdit.dart';
import 'SampleDetailsProvider.dart';

class SampleDetailsList extends StatefulWidget {
  const SampleDetailsList({super.key});

  @override
  State<SampleDetailsList> createState() => _SampleDetailsListState();
}

class _SampleDetailsListState extends State<SampleDetailsList> {
  ...
  Widget build(BuildContext context) {
    final ticketProvider = Provider.of<SampleDetailsProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('Sample Details Page'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, SampleDetailsAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildTicketList(List<SampleDetails> tickets) {
  ...
  Widget _buildTicketCard(SampleDetails ticket, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
