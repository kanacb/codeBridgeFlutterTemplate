...
import 'StockInDetailsAdd.dart';
import 'StockInDetailsEdit.dart';
import 'StockInDetailsProvider.dart';

class StockInDetailsList extends StatefulWidget {
  const StockInDetailsList({super.key});

  @override
  State<StockInDetailsList> createState() => _StockInDetailsListState();
}

class _StockInDetailsListState extends State<StockInDetailsList> {
  ...
  Widget build(BuildContext context) {
    final ticketProvider = Provider.of<StockInDetailsProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('Stock In Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, StockInDetailsAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildTicketList(List<StockInDetails> tickets) {
  ...
  Widget _buildTicketCard(StockInDetails ticket, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ListTile(
            ...
            subtitle: Column(
              ...
              children: [
                Text('Pricing: ${ticket.pricing}'),
                Text('Quantity: ${ticket.quantity}'),
                if (_showMore[index])
                  Text('Serial No: ${ticket.serialNo}'),
                  Text('Part No: ${ticket.partNo}'),
                  Text('Purchased Date: ${ticket.purchaseDate}'),
                Text('Part Description: ${ticket.partDescription}'),
                Text('PO Number: ${ticket.poNumber}'),
                Text('DO Number: ${ticket.doNumber}'),
                Text('Category: ${ticket.category}'),
                Text('Unit of Measurement: ${ticket.unitOfMeasurement}'),
                Text('Condition of Terms: ${ticket.conditionOfTerms}'),
                Text('Warehouse: ${ticket.warehouse}'),
                  ...
                  onPressed: () {
                    showDialog<String>(
                      ...
                      builder: (context) => DeleteDialog(
                        ...
                        neg: "Cancel",
                        id: ticket.id,
                        answer: (answer) => _deleteTicket(ticket.id, answer),
                      ),
                    );
                  },
                ),
