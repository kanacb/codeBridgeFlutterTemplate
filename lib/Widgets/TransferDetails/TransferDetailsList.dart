...
import 'TransferDetailsAdd.dart';
import 'TransferDetailsEdit.dart';
import 'TransferDetailsProvider.dart';

class TransferDetailsList extends StatefulWidget {
  const TransferDetailsList({super.key});

  @override
  State<TransferDetailsList> createState() => _TransferDetailsListState();
}

class _TransferDetailsListState extends State<TransferDetailsList> {
  ...
  Widget build(BuildContext context) {
    final ticketProvider = Provider.of<TransferDetailsProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('Transfer Details Page'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, TransferDetailsAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildTicketList(List<TransferDetails> tickets) {
  ...
  Widget _buildTicketCard(TransferDetails ticket, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ListTile(
            ...
            title: Text(
              'Part Number: ${ticket.partNumber}',
              style: const TextStyle(fontWeight: FontWeight.bold),
            ),
            subtitle: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Source Warehouse: ${ticket.sourceWarehouse}'),
                Text('Destination Warehouse: ${ticket.destinationWarehouse}'),
                Text('Quantity: ${ticket.quantity}'),
                Text('Transfer Date: ${ticket.transferDate}'),
                Text('Transfer Status: ${ticket.transferStatus}'),
