...
class VendingMachineAdd extends StatefulWidget {
  const VendingMachineAdd({super.key, required this.resource});
  final List<Schema> resource;

  @override
  State<VendingMachineAdd> createState() => _VendingMachineAddState();
}

class _VendingMachineAddState extends State<VendingMachineAdd> {
  ...
  void saveForm() async {
    ...
    VendingMachineProvider machineMasterProvider = VendingMachineProvider();
    // Handle form submission logic here
    final data = VendingMachine.fromJson(formData);
    Response response = await machineMasterProvider.createOneAndSave(data);
    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully created machine master");
    } else {
      snackBar.FailedSnackBar(context, "Failed to create machine master");
    }
  }
