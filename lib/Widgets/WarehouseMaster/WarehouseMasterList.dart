...
import 'WarehouseMasterAdd.dart';
import 'WarehouseMasterEdit.dart';
import 'WarehouseMasterProvider.dart';

class WarehouseMasterList extends StatefulWidget {
  const WarehouseMasterList({super.key});

  @override
  State<WarehouseMasterList> createState() => _WarehouseMasterListState();
}

class _WarehouseMasterListState extends State<WarehouseMasterList> {
  ...
  Widget build(BuildContext context) {
    final ticketProvider = Provider.of<WarehouseMasterProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('Warehouse Master Page'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, WarehouseMasterAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildTicketList(List<WarehouseMaster> tickets) {
  ...
  Widget _buildTicketCard(WarehouseMaster ticket, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ListTile(
            ...
            title: Text(
              'Warehouse Name: ${ticket.name}',
              style: const TextStyle(fontWeight: FontWeight.bold),
            ),
            subtitle: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Warehouse Location: ${ticket.location}'),
              ],
            ),
            trailing: Row(
