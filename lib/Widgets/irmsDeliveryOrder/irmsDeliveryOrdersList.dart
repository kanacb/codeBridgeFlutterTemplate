...
import 'irmsDeliveryOrdersAdd.dart';
import 'irmsDeliveryOrdersEdit.dart';
import 'irmsDeliveryOrdersProvider.dart';
import 'irmsDeliveryOrders.dart';
import 'irmsDeliveryOrdersProvider.dart';

class irmsDeliveryOrdersList extends StatefulWidget {
  const irmsDeliveryOrdersList({super.key});

  @override
  State<irmsDeliveryOrdersList> createState() => _irmsDeliveryOrdersListState();
}

class _irmsDeliveryOrdersListState extends State<irmsDeliveryOrdersList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await irmsDeliveryOrdersProvider().schema();
    setState(() {});
  }

  Future<void> _deleteTicket(String id, bool confirm) async {
    if (confirm) {
      final provider = Provider.of<irmsDeliveryOrdersProvider>(context, listen: false);
      Response response = await provider.deleteOne(id);

      if (response.isSuccess) {
        SnackBars().SuccessSnackBar(context, "Deleted ticket successfully");
        setState(() {
          provider.loadirmsDeliveryOrdersFromHive();
        });
      } else {
  ...
  Widget build(BuildContext context) {
    final ticketProvider = Provider.of<irmsDeliveryOrdersProvider>(context);
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, irmsDeliveryOrdersAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildTicketList(List<irmsDeliveryOrders> tickets) {
  ...
  Widget _buildTicketCard(irmsDeliveryOrders ticket, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ListTile(
            ...
            title: Text(
              'Delivery Order ID: ${ticket.deliveryOrderId}',
              style: const TextStyle(fontWeight: FontWeight.bold),
            ),
            subtitle: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Purchase Order: ${ticket.purchaseOrder}'),
              ],
            ),
            trailing: Row(
                  ...
                  onPressed: () {
                    showDialog<String>(
                      ...
                      builder: (context) => DeleteDialog(
                        ...
                        pos: "Yes",
                        neg: "Cancel",
                        id: ticket.deliveryOrderId,
                        answer: (answer) => _deleteTicket(ticket.deliveryOrderId, answer),
                      ),
                    );
                  },
                ),
