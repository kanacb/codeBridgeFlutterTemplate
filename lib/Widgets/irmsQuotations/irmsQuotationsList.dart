...
import 'irmsQuotationsAdd.dart';
import 'irmsQuotationsEdit.dart';
import 'irmsQuotationsProvider.dart';

class irmsQuotationsList extends StatefulWidget {
  const irmsQuotationsList({super.key});

  @override
  State<irmsQuotationsList> createState() => _irmsQuotationsListState();
}

class _irmsQuotationsListState extends State<irmsQuotationsList> {
  ...
  Widget build(BuildContext context) {
    final ticketProvider = Provider.of<irmsQuotationsProvider>(context);
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, irmsQuotationsAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildTicketList(List<irmsQuotations> tickets) {
  ...
  Widget _buildTicketCard(irmsQuotations ticket, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ListTile(
            ...
            title: Text(
              'Sales Order Number: ${ticket.salesOrder}',
              style: const TextStyle(fontWeight: FontWeight.bold),
            ),
            subtitle: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Valid Date: ${ticket.validDate}'),
                Text('Quotation Index: ${ticket.quotationIndex}'),
              ],
            ),
            trailing: Row(
                  ...
                  onPressed: () {
                    showDialog<String>(
                      ...
                      builder: (context) => DeleteDialog(
                        ...
                        pos: "Yes",
                        neg: "Cancel",
                        id: ticket.salesOrder,
                        answer: (answer) => _deleteTicket(ticket.salesOrder, answer),
                      ),
                    );
                  },
                ),
